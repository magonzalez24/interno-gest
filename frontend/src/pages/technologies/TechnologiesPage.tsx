...
export const TechnologiesPage = () => {
  ...
  const [employeeCountByTech, setEmployeeCountByTech] = useState<Record<string, number>>({});
      ...
      try {
        await Promise.all(
          technologies.map(async (tech) => {
            try {
              const stats = await api.getTechnologyStats(tech.id);
              counts[tech.id] = stats.employeeCount;
            } catch (error) {
              console.error(`Error loading stats for ${tech.id}:`, error);
              counts[tech.id] = 0;
            }
          })
        );
        setEmployeeCountByTech(counts);
      } catch (error) {
